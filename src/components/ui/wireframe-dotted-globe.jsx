...
export default function RotatingEarth({ width = 800, height = 600, className = "" }) {
  ...
  useEffect(() => {
    ...
    const handleWheel = (event) => {
      ...
      projection.scale(Math.max(radius * 0.5, Math.min(radius * 3, projection.scale() * scaleFactor)))
      render()
    }
  ...
  if (error) {
    return (
      <div className={`flex items-center justify-center rounded-lg p-8 border border-white/10 ${className}`}>
        <p className="text-xs text-white/30 uppercase tracking-widest">{error}</p>
      </div>
    )
  }
