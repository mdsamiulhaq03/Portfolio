...
const TextPressure = ({
  ...
  useEffect(() => {
    ...
    const handleTouchMove = e => { const t = e.touches[0]; cursorRef.current.x = t.clientX; cursorRef.current.y = t.clientY; };
    ...
    const animate = () => {
      ...
      if (titleRef.current) {
        ...
        spansRef.current.forEach(span => {
          ...
          if (span.style.fontVariationSettings !== newSettings) span.style.fontVariationSettings = newSettings;
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-transparent">
      ...
      <h1
        ref={titleRef}
        className={`${className} ${flex ? 'flex justify-between' : ''} ${stroke ? 'stroke' : ''} uppercase text-center`}
        style={{
          fontFamily,
          fontSize,
          lineHeight,
